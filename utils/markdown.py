...
def clean_md_document(input_fp: Path, actions_id: str = ACTIONS_ID, summary_id: str = SUMMARY_ID) -> None:
    ...
    md_document = _replace_md_data(md_document, actions_id, md_data)
    md_document = _update_summary_action(md_document, summary_id, "action-count", str(len(df)))
    input_fp.write_text(md_document)
